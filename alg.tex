...
一般的，有三种符号来表示算法的复杂度：
\begin{itemize}
    \item 大O符号（$O$）：表示算法的上界，描述在最坏情况下的复杂度。
    \item 大Omega符号（$\Omega$）：表示算法的下界，描述在最好情况下的复杂度。
    \item 大Theta符号（$\Theta$）：表示算法的紧确界，描述在平均情况下的复杂度。
\end{itemize}
一般的，我们用n表示问题的规模，而这些符号后面跟着的函数则表示随着n的增长，算法复杂度的增长趋势。例如，$O(n^2)$表示算法的时间复杂度在最坏情况下是平方级别的，而具体的常数和低阶项等则被忽略。
...
递推指的是通过已知的结果，推导出未知的结果。动态规划是一种将复杂问题分解为更简单子问题的方法。它适用于\textbf{问题可以分解为重叠子问题}、\textbf{子问题的最优解可以构成原问题的最优解}的场景。动态规划通常通过构建一个表格来存储子问题的解，从而避免重复计算，提高效率。

几乎100\%的递推问题都是动态规划问题，或者可以通过动态规划来节省运算时间。
...
\begin{soln}
    该问题可以通过动态规划来解决。我们可以定义一个数组\texttt{dp}，其中\texttt{dp[i]}表示爬到第$i$级台阶的不同方式数。显然，\texttt{dp[0]=1}（不需要爬任何台阶），\texttt{dp[1]=1}（只有一种方式爬到第一级台阶）。对于$i \geq 2$，我们可以通过以下递推关系来计算\texttt{dp[i]}：
    $$dp[i] = dp[i-1] + dp[i-2]$$
...
有些动态规划问题比较困难，需要仔细分析问题，找出合适的状态和递推关系。
