...
\section[最短路算法专题]{最短路算法专题\protect\footnote{本章作者王煜程。}}

最短路问题是图论中的一个经典问题，旨在寻找图中两个节点之间的最短路径。根据问题的不同，可以分为单源最短路和多源最短路两大类。
...
DFS,BFS求单源最短路的时间复杂度为$O(V+E)$，但是只能用于无权图中。

在使用过程中需要注意进行节点的标记，显然的是，两次重复访问同一个节点，将中间的路程删掉是一定更短的。

具体实现可以参照搜索专题中的DFS,BFS。

\subsubsection{Dijkstra算法}

Dijstra算法是最常用的单源最短路算法之一，也是平均性能最好的单源最短路算法，适用于边权非负的图。

Dijstra算法是贪心算法的一个典型应用，以局部最优解组成全局最优解。

其具体的实现过程如下：

1.取未确定距离点集中距离源点最近的点，对与其相邻的点进行松弛操作
...
\begin{example}
  ...
  \textbf{输入}：第一行为三个正整数$n$,$m$,$s$。第二行起$m$行，每行三个非负整数$u_i$,$v_i$,$w_i$，表示$u_i$到$v_i$有一条权值为$w_i$的有向边。

  \textbf{输出}：输出$n$个非负整数，表示从$s$到每个点的最短路距离。

\end{example}

\begin{soln}

  显然，这是一个非常模板的单源最短路问题，我们可以直接使用Dijkstra算法解决。
...
多源最短路问题是指在一个图中，找到所有节点对之间的最短路径。常见的多源最短路算法包括Floyd算法等。

\subsubsection{Floyd算法}

Floyd算法是解决多源最短路最经典的算法之一，适用于边权可以为负的图，也可用于判断图中是否含有负权环。

Floyd算法的核心思想是动态规划，设$dis[i][j]$表示$i$到$j$的最短路距离，初始时$dis[i][j]$为边权，如果$i$到$j$没有边，则为无穷大。

设$k$为中间节点，则有状态转移方程：
$$dis[i][j] = min(dis[i][j],dis[i][k]+dis[k][j])$$

这里就可以用3层for循环进行解决，值得注意的是，$k$的循环必须放在最外层。
...
\begin{problem}
  ...
  给定一个由$n$个点，$m$条边组成的无向图。

  求出所有($i$,$j$)点对之间的最短路径。

  textbf{输入}：第一行为两个正整数$n$,$m$。第二行起$m$行，每行三个整数$u_i$,$v_i$,$w_i$，表示$u_i$到$v_i$有一条权值为$w_i$的无向边。

  textbf{输出}：输出$n$行，每行$n$个整数，表示每个点对之间的最短路距离。
\end{problem}
